...
const NUMBER_RE = /(?:여덟|여섯|네|두|팔|육|사|이)(?=\s*(?:컷|장|가지))/gu;
const NORMALIZATION = [
  'Ignore whitespace and punctuation; Latin letter case is ignored.',
  'Only Korean 2/4/6/8 number forms immediately before 컷, 장 or 가지 are mapped to digits.',
...
function buildTiming(asr, script, master, options = {}) {
  ...
  for (let i = scenes.length - 1; i >= 0; i--) {
    ...
    if (shot.id === 'layout' && shot.variant === 'counts') {
      const normPhrase = normalize(phrase.text);
      const variants = [['two', '2컷'], ['four', '4컷'], ['six', '6컷']];
      const starts = variants.map(([variant, token], index) => {
        const unitIndex = normPhrase.indexOf(token);
        assert(unitIndex >= 0, 'Missing spoken layout count ' + token);
        return index === 0 ? shot.start : round(tokenForUnit(phrase.normalizedStart + unitIndex).dtwSeconds + offset - lead);
      });
      scenes.splice(i, 1, ...variants.map(([variant], index) => ({
        ...shot, variant, start: starts[index], end: starts[index + 1] ?? shot.end,
      })));
    } else if (shot.id === 'layout' && phrase.text.includes('필터')) {
  ...
  return { report, story: {
    ...
    note: 'Estimated phrase timing. Layout-count cuts follow the actual spoken 2/4/6 DTW token moments. No exact voice-similarity claim.',
    scenes,
  } };
}
...
function main() {
  ...
  const asrFile = value('--asr', path.join(ROOT, 'build/reels-seasonal-v4/narration-whisper-local.json'));
