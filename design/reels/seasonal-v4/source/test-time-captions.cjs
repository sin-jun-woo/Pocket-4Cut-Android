...
test('only contextual number spelling is normalized; brand errors remain visible', () => {
  ...
  assert.equal(compareText('여덟 가지 배치', '8가지 배치').normalizedTranscriptEqualsExpected, true);
});
...
test('seasonal 2/4/6 cut variants follow the spoken count tokens', () => {
  const phrase = { id: 'layout', variant: 'counts', text: '두 컷, 네 컷, 여섯 컷.', lines: ['2컷 · 4컷 · 6컷'] };
  const data = {
    result: { language: 'ko' }, model: { type: 'synthetic' },
    transcription: [{ text: ' 두 컷, 네 컷, 여섯 컷.', offsets: { from: 100, to: 2500 },
      tokens: [
        { id: 100, text: ' 두 컷,', t_dtw: 10, offsets: { from: 100, to: 800 } },
        { id: 101, text: ' 네 컷,', t_dtw: 90, offsets: { from: 900, to: 1600 } },
        { id: 102, text: ' 여섯 컷.', t_dtw: 170, offsets: { from: 1700, to: 2500 } },
      ] }],
  };
  const { report, story } = buildTiming(data, { text: phrase.text, phrases: [phrase] }, { durationSeconds: 3, voiceTimeOffsetSeconds: .12 });
  assert.equal(report.readyForRender, true);
  assert.deepEqual(story.scenes.map(s => s.variant), ['two', 'four', 'six']);
  assert.deepEqual(story.scenes.map(s => [s.start, s.end]), [[0, .97], [.97, 1.77], [1.77, 3]]);
  assert.equal(story.scenes[0].lines[0], '2컷 · 4컷 · 6컷');
});
