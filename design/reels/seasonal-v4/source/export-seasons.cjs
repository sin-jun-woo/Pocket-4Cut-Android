...
const VISUAL=path.join(ROOT,'build/reels-render/seasonal-v4-visual.mp4'),AUDIO=path.join(BASE,'audio/narration-master.wav');
const VIDEO=path.join(OUT,'Pocket4Cut-Reels-Seasons-Voice.mp4');
...
async function main(){
  ...
  ff(['-v','error','-y','-i',AUDIO,'-c:a','libmp3lame','-b:a','192k','-map_metadata','-1',path.join(OUT,'Pocket4Cut-Seasons-Narration.mp3')]);
  ...
  const cover=await loadImage(path.join(OUT,'Pocket4Cut-Seasons-Cover.jpg'));assert.equal(cover.width,1080);assert.equal(cover.height,1920);
