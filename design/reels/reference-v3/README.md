...
광고·AI 사용 예시 표시는 참조 출처 크레딧과 구분해 유지한다. 자세한 편집 의도는 `source/CREATIVE-BRIEF.md`에 있다.

## 음성과 검증

실제 결과: 20.160초 원본 음성을 23초 마스터로 정리했다. 자동 전사는 숫자·공백·구두점 정규화 후 대본과 일치했다. 14장면/690프레임 최종 MP4의 전체 디코딩·규격·동기·faststart·음량 검사를 통과했고, 검은 구간은 없었다. 최종 AAC는 −16.0 LUFS / −3.4 dBTP다. 28개 시점 스틸과 핵심 텍스트 경계 70개를 검사하고 최종 추출 프레임·커버를 시각 검토했다. ZIP의 7개 항목 모두 원본 해시와 일치했다.
