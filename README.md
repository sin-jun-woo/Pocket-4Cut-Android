...
앨범 가져오기는 Android의 시스템 사진 선택 도구를 사용하며 전체 사진 읽기 권한을 요청하지 않습니다. 선택한 외부 원본은 수정하거나 삭제하지 않고, 바이트를 앱 전용 공간에 복사한 뒤 형식·크기·실제 픽셀 decode를 검증해 그 사본만 세션에 연결합니다. 가져오기를 취소하거나 일부만 성공해도 완료된 사본과 순서는 초안으로 보존됩니다. 프로세스가 중단되면 홈과 작업 보관함이 세션 목록을 읽기 전에 저장된 import/제거 journal을 재생합니다.
...
진행 중인 작업은 초안으로 보존합니다. 홈에서 최근 작업을 이어가거나 작업 보관함의 진행 중인 작업 목록에서 재개·삭제할 수 있습니다. 손상된 미완료 결과와 세션 문서는 작업 보관함에서 구분해 보여 주며, 확인한 결과 후보만 격리하거나 이전 정상본 복구를 시도할 수 있습니다. 정상본이 없어 읽을 수 없는 세션은 원본 파일을 보존한 채 작업 보관함에서 숨길 수 있고, 숨긴 기록 관리에서 다시 표시할 수 있습니다. 이때 파일 소유권을 확인할 수 없으므로 숨긴 기록이 있는 동안 다른 세션의 실제 파일 삭제는 중단됩니다. 기록된 촬영 원본이나 가져온 사본이 누락·손상됐을 때는 자동으로 다른 사진을 대입하지 않고 작업을 멈춥니다. 작업을 버리는 행동과 일반 뒤로가기는 구분합니다.
...
- **작업 보관함 삭제:** 앱이 소유한 세션·초안·촬영 원본·앨범 import 사본을 정리합니다. 외부 앨범 원본과 이미 사진첩에 내보낸 공용 사본은 유지합니다. 전체 삭제는 진행 중인 작업도 포함하며 확인창에서 범위를 표시합니다.
...
- 앱 전용 Pictures: 촬영 JPEG 원본, 앨범에서 바이트 그대로 복사한 import 원본, 완성 결과를 서로 다른 디렉터리에 저장합니다. 원본을 축소·재압축해 덮어쓰지 않으며 결과는 별도 파일로 만듭니다.
...
- 앱의 최소 OS는 Android 8/API 26, compile/target SDK는 36입니다. 현재 버전은 1.5(6)이며 이번 기능 구현에서는 버전을 올리지 않았습니다. 실제 값은 [app/build.gradle.kts](app/build.gradle.kts)가 기준입니다.
