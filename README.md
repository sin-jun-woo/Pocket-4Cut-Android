...
이 문서는 **2026-09-22 신뢰성 개편 코드 커밋 `155ced0`** 및 후속 **Paper Seasons 변경(`d249a01`에서 시작)**을 반영합니다. 구현된 경로와 검증 완료 범위는 다릅니다. 실행한 검사·남은 문제·커밋 기준은 [작업 기록](docs/WORKLOG.md)을 확인하세요. 초기 기획 문서의 기술 스택이나 일정은 현재 구현의 근거가 아닙니다.
...
진행 중인 작업은 초안으로 보존합니다. 홈에서 최근 작업을 이어가거나 보관함의 진행 중인 작업 목록에서 재개·삭제할 수 있습니다. 손상되었거나 사진을 찾을 수 없는 작업은 자동 재개 대신 복구 필요 상태로 표시합니다. 작업을 버리는 행동과 일반 뒤로가기는 구분합니다.
...
[Android CI](.github/workflows/android-ci.yml)는 빌드·JVM 테스트·Lint·최적화 QA APK와 API 28/36 에뮬레이터 계측을 정의합니다. 워크플로 정의와 실제 원격 실행 성공은 구분해야 합니다.
